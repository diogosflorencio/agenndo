  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        .from("appointments")
        .select(
          "id, collaborator_id, client_id, date, time_start, time_end, price_cents, status, payment_status, payment_due_cents, payment_collected_cents, client_name_snapshot, service_variant_label, appointment_payments(provider_payment_id, status, amount_cents, payment_kind), clients(name, phone), services(name), collaborators(id, name)"
        )
