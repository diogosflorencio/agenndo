...
// Menu agrupado: Agenda, Cadastros, Dados + itens diretos
const MENU_AGENDA = [
...
const DIRECT_LINKS = [
  { href: "/dashboard/conta", icon: "manage_accounts", label: "Conta" },
];

// Mobile: mesma estrutura do PC — cada item na barra; grupos exibem subitens acima ao tocar
type MobileNavItem =
...
function DashboardLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (
        refs.agenda.current && !refs.agenda.current.contains(target) &&
        refs.cadastros.current && !refs.cadastros.current.contains(target) &&
        refs.dados.current && !refs.dados.current.contains(target) &&
        refs.config.current && !refs.config.current.contains(target)
      ) {
        setOpenGroup(null);
      }
    }
  ...
  const renderGroup = (
    ...
    return (
      <div className="relative" ref={ref}>
        ...
        {open && (
          <div className={`absolute top-full left-0 mt-1 py-1 min-w-[200px] rounded-xl border shadow-lg z-50 ${
            isLight ? "bg-white border-gray-200" : "bg-[#0f1c15] border-white/10"
          }`}>
            {items.map((item) => (
              <NavItem
                key={item.href}
  ...
  return (
    <div className={`min-h-screen flex flex-col ${bgMain}`} data-theme={theme}>
          ...
          <div className="flex items-center justify-between h-14 lg:h-16">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Link
                ...
                className="lg:hidden size-9 flex items-center justify-center rounded-lg relative"
              >
      ...
      {/* Mobile: subitens do grupo expandido — exibidos logo acima da barra (botão tamanho do conteúdo) */}
      {mobileExpandedGroup && (
        <div
          className={`lg:hidden fixed left-0 right-0 z-30 border-t px-2 py-2 flex flex-wrap gap-1.5 justify-center ${navBottomBg}`}
        ...
        >
          {(mobileExpandedGroup === "agenda" ? MENU_AGENDA : mobileExpandedGroup === "cadastros" ? MENU_CADASTROS : mobileExpandedGroup === "dados" ? MENU_DADOS : MENU_CONFIG).map((item) => (
            <Link
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap ${
                isActive(item.href) ? "bg-primary/20 text-primary" : isLight ? "bg-gray-100 text-gray-700" : "bg-white/10 text-gray-300"
              }`}
            >
        ...
        {MOBILE_NAV_ITEMS.map((item) => {
          if (item.type === "link") {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setMobileExpandedGroup(null)}
                className={`flex flex-col items-center gap-0.5 py-1.5 flex-1 min-w-0 max-w-[72px] ${active ? "text-primary" : isLight ? "text-gray-500" : "text-gray-500"}`}
              >
          ...
          return (
            <button
              key={item.key}
              type="button"
              onClick={() => setMobileExpandedGroup(open ? null : item.key)}
