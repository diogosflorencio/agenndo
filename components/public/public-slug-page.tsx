...
export type PublicPageInnerProps = {
  ...
  /** Query `?collab=` (`any` ou id do colaborador) */
  prefillCollaboratorId?: string | null;
...
export function PublicPageInner({
  ...
  /** Mesma query da barra de endereço (evita perder `service`/`collab`/… se props atrasarem). */
  const urlBookingQuery = useMemo(() => parsePublicBookingQuery(searchParams), [searchParams]);
  const effPrefillServiceId =
    entry === "booking" ? urlBookingQuery.serviceId ?? prefillServiceId : null;
  const effPrefillCollaboratorId =
    entry === "booking" ? urlBookingQuery.collaboratorId ?? prefillCollaboratorId : null;
  ...
  const prefillTimeAppliedRef = useRef<string | null>(null);
  /** Evita re-sincronizar o passo com a URL no mesmo segmento (ex.: após `goBookingStep` antes do router atualizar). */
  const urlStepSegmentRef = useRef<string | null>(null);

  const bookingQuerySnapshot = useCallback(
    (overrides?: {
      ...
      const collabFromState =
        selectedCollab === "any" ? "any" : selectedCollab?.id ?? null;
      return buildPublicBookingQuery({
        serviceId:
          overrides && "serviceId" in overrides
            ? overrides.serviceId
            : selectedService?.id ?? effPrefillServiceId ?? null,
        collaboratorId:
          overrides && "collaboratorId" in overrides
            ? overrides.collaboratorId
            : collabFromState ?? effPrefillCollaboratorId ?? null,
        date:
          overrides && "date" in overrides
            ? overrides.date
            : selectedDate ?? effPrefillDate ?? null,
        time:
          overrides && "time" in overrides
            ? overrides.time
            : selectedTime ?? effPrefillTime ?? null,
      });
    },
    [
      selectedService?.id,
  ...
  useEffect(() => {
    urlStepSegmentRef.current = null;
  }, [slug]);
  ...
  useEffect(() => {
    ...
    if (parsed === null) {
      ...
      void router.replace(`/${slug}/agendar/servico${qs}`, { scroll: false });
      return;
    }
    if (urlStepSegmentRef.current === bookingStepSegment) return;
    urlStepSegmentRef.current = bookingStepSegment;
    setStep(parsed);
  }, [bookingStepSegment, entry, slug, router]);

  useEffect(() => {
    if (entry !== "booking" || !effPrefillServiceId || !services.length || loading) return;
    if (prefillAppliedRef.current === effPrefillServiceId) return;
    const svc = services.find((s) => s.id === effPrefillServiceId);
    if (!svc) return;
    prefillAppliedRef.current = effPrefillServiceId;
  ...
  }, [entry, effPrefillServiceId, services, loading]);

  useEffect(() => {
    if (entry !== "booking" || !effPrefillCollaboratorId || !collaborators.length || loading) return;
    if (prefillCollabAppliedRef.current === effPrefillCollaboratorId) return;
    prefillCollabAppliedRef.current = effPrefillCollaboratorId;
    if (effPrefillCollaboratorId === "any") {
      setSelectedCollab("any");
      return;
    }
    const c = collaborators.find((x) => x.id === effPrefillCollaboratorId);
    if (c) setSelectedCollab(c);
  }, [entry, effPrefillCollaboratorId, collaborators, loading]);

  useEffect(() => {
    if (entry !== "booking" || !effPrefillDate || loading) return;
    if (prefillDateAppliedRef.current === effPrefillDate) return;
    prefillDateAppliedRef.current = effPrefillDate;
    setSelectedDate(effPrefillDate);
  }, [entry, effPrefillDate, loading]);

  useEffect(() => {
    if (entry !== "booking" || !effPrefillTime || loading) return;
    if (prefillTimeAppliedRef.current === effPrefillTime) return;
    prefillTimeAppliedRef.current = effPrefillTime;
    setSelectedTime(effPrefillTime);
  }, [entry, effPrefillTime, loading]);

  useEffect(() => {
    if (entry !== "booking" || !slug || loading) return;
    if (step >= 2 && !selectedService) {
      if (effPrefillServiceId) {
        const exists = services.some((s) => s.id === effPrefillServiceId);
        if (!exists && services.length > 0) {
          goBookingStep(1, {
            replace: true,
            collaboratorId: null,
            date: null,
      ...
      goBookingStep(1, { replace: true, collaboratorId: null, date: null, time: null });
      return;
    }
    if (step >= 3 && selectedCollab === null) {
      if (effPrefillCollaboratorId) return;
      goBookingStep(2, { replace: true, collaboratorId: null, date: null, time: null });
      return;
    }
    if (step >= 4 && !selectedDate) {
      if (effPrefillDate) return;
      goBookingStep(3, { replace: true, date: null, time: null });
      return;
    }
    if (step >= 5 && !selectedTime) {
      if (effPrefillTime) return;
      goBookingStep(4, { replace: true, time: null });
    }
  }, [
    entry,
    slug,
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (step >= 4) goBookingStep(3, { replace: true, date: null, time: null });
    }
  ...
  const resetWizardState = useCallback(() => {
    ...
    prefillCollabAppliedRef.current = null;
    prefillDateAppliedRef.current = null;
    prefillTimeAppliedRef.current = null;
  }, []);
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (v.length > 0 || desc) {
                      ...
                      goBookingStep(2, { serviceId: service.id });
                    }
                  }}
                  className={cn(
                    "flex flex-row items-center gap-3 p-3 rounded-xl border text-left transition-all hover:-translate-y-0.5",
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              <button
                ...
                onClick={() => {
                  setSelectedCollab("any");
                  goBookingStep(3, { collaboratorId: "any" });
                }}
                className={cn(
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      goBookingStep(3, { collaboratorId: collab.id });
                    }}
                    className={cn(
                      "flex items-center gap-4 p-4 rounded-2xl border hover:border-[color-mix(in_srgb,var(--public-accent)_35%,transparent)] text-left transition-all",
                      bookUi.card,
            ...
            left={
              <PublicDatePicker
                ...
                onSelectDate={(dateStr) => {
                  ...
                  setSelectedTime(null);
                  goBookingStep(4, { date: dateStr, time: null });
                }}
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const t = minutesToTime(sliderStartMin);
                    setSelectedTime(t);
                      ...
                      {cells.map((cell) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            goBookingStep(5, { time: cell.start });
                          }}
                          className={cn(
